...
public class FullOuterJoin {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			System.out.println("FullOuterJoin BEGIN");
			...
			// Perform Full Outer Joint
			Table fullOuterJoin = customers
					.fullOuterJoin(orders,"customer_id=customer_key")
					.select("first_name,last_name,email,address,city,state,zip,order_date,amount");
		
			// Write to Sinks
			int parallelism = 1;
			
			// Write to Sinks
			fullOuterJoin.printSchema();
//			DataSet<Row> result = tableEnv.toDataSet(leftOuterJoin, Row.class);
//			result.print();
			
			TableSink<Row> joinSink = new CsvTableSink("output/fullouterjoin.csv", ",", parallelism, WriteMode.OVERWRITE);
			fullOuterJoin.writeToSink(joinSink);
				
					
			// Execute
			JobExecutionResult result  =  env.execute("FullOuterJoin");
