...
public class UnionAll {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			System.out.println("UnionAll BEGIN");
			...
			// Perform Union
			Table allCustomers = customerset1.unionAll(customerset2);
			...
			// Write to Sinks
			allCustomers.printSchema();
			DataSet<Row> result = tableEnv.toDataSet(allCustomers, Row.class);
			result.print();
			
			TableSink<Row> allSink = new CsvTableSink("output/unionall_customers.csv", ",", parallelism, WriteMode.OVERWRITE);
			allCustomers.writeToSink(allSink);
